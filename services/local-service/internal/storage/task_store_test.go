...
func TestTaskStoresSupportGetAndListBySession(t *testing.T) {
	...
	if err := inMemory.WriteTask(context.Background(), TaskRecord{TaskID: "task_mem_001", SessionID: "sess_001", Title: "Task A", UpdatedAt: "2026-04-21T10:02:00Z", StartedAt: "2026-04-21T10:00:00Z"}); err != nil {
		t.Fatalf("in-memory WriteTask returned error: %v", err)
	}
	if err := inMemory.WriteTask(context.Background(), TaskRecord{TaskID: "task_mem_002", SessionID: "sess_001", Title: "Task B", UpdatedAt: "2026-04-21T10:03:00Z", StartedAt: "2026-04-21T10:01:00Z"}); err != nil {
		t.Fatalf("in-memory WriteTask returned error: %v", err)
	}
	record, err := inMemory.GetTask(context.Background(), "task_mem_001")
	if err != nil || record.Title != "Task A" {
	...
	if err := sqliteStore.WriteTask(context.Background(), TaskRecord{TaskID: "task_sql_001", SessionID: "sess_sql", Title: "Task SQL", Status: "processing", UpdatedAt: "2026-04-21T10:05:00Z", StartedAt: "2026-04-21T10:00:00Z"}); err != nil {
		t.Fatalf("sqlite WriteTask returned error: %v", err)
	}
	record, err = sqliteStore.GetTask(context.Background(), "task_sql_001")
	if err != nil || record.Title != "Task SQL" {
